...
export const fetchGameById = (gameId: number): Promise<{data: Game; message: string}> => get<{ data: Game; message: string }>(`/games/${gameId}`);

export const startGame = async (gameId: number): Promise<{data: SessionDto; message: string}> => post<{data: SessionDto; message: string}>(`/games/start/${gameId}`);
